...
namespace FaultyBot.Services.Impl
{
    public class BotCredentials : IBotCredentials
    {
        ...
        public BotCredentials()
        {
            ...
            if(!File.Exists(credsFileName))
                _log.Warn($"credentials.json is missing. Attempting to load creds from environment variables prefixed with 'FaultyBot_'. Example is in {Path.GetFullPath("./credentials_example.json")}");
            try
            {
                var configBuilder = new ConfigurationBuilder();
                configBuilder.AddJsonFile(credsFileName, true)
                    .AddEnvironmentVariables("FaultyBot_");
        ...
        private class CredentialsModel
        {
            ...
            public DBConfig Db { get; set; } = new DBConfig("sqlite", "Filename=./data/FaultyBot.db");
