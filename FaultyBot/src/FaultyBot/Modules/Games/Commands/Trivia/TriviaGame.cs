...
namespace FaultyBot.Modules.Games.Trivia
{
    public class TriviaGame
    {
        ...
        private async Task StartGame()
        {
            while (!ShouldStopGame)
            {
                ...
                //receive messages
                FaultyBot.Client.MessageReceived += PotentialGuess;
            ...
            try { FaultyBot.Client.MessageReceived -= PotentialGuess; } catch { }
            GameActive = false;
            await End().ConfigureAwait(false);
        }
        ...
        private Task PotentialGuess(IMessage imsg)
        {
            ...
            var t = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (umsg.Author.Id == FaultyBot.Client.GetCurrentUser().Id) return;
