    ...
    public partial class Games
    {
        ...
        public class PlantPickCommands
        {
            ...
            public PlantPickCommands()
            {
                ...
                FaultyBot.Client.MessageReceived += PotentialFlowerGeneration;
                rng = new FaultyRandom();

                using (var uow = DbHandler.UnitOfWork())
                {
                    var conf = uow.BotConfig.GetOrCreate();
                    var x =
                    generationChannels = new ConcurrentHashSet<ulong>(FaultyBot.AllGuildConfigs
