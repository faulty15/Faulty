...
namespace FaultyBot.Modules.NSFW
{
    [FaultyModule("NSFW", "~")]
    public class NSFW : DiscordModule
    {
        ...
        public async Task Boobs(IUserMessage umsg)
        {
            ...
            try
            {
                ...
                using (var http = new HttpClient())
                {
                    obj = JArray.Parse(await http.GetStringAsync($"http://api.oboobs.ru/boobs/{ new FaultyRandom().Next(0, 9880) }").ConfigureAwait(false))[0];
                }
        ...
        public async Task Butts(IUserMessage umsg)
        {
            ...
            try
            {
                ...
                using (var http = new HttpClient())
                {
                    obj = JArray.Parse(await http.GetStringAsync($"http://api.obutts.ru/butts/{ new FaultyRandom().Next(0, 3873) }").ConfigureAwait(false))[0];
                }
