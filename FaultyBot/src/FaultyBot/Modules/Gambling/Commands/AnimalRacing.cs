...
namespace FaultyBot.Modules.Gambling
{
            ...
            public class AnimalRace
            {
                ...
                public AnimalRace(ulong serverId, ITextChannel ch)
                {
                    ...
                    Task.Run(async () =>
                    {
                        try
                        {
                            try { await raceChannel.SendMessageAsync($"🏁`Race is starting in 20 seconds or when the room is full. Type {FaultyBot.ModulePrefixes[typeof(Gambling).Name]}jr to join the race.`"); } catch (Exception ex) { _log.Warn(ex); }
