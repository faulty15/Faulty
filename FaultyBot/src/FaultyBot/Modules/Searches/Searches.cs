...
using System.IO;
using FaultyBot.Modules.Searches.Commands.OMDB;

namespace FaultyBot.Modules.Searches
{
    [FaultyModule("Searches", "~")]
    public partial class Searches : DiscordModule
    {
        ...
        public async Task Weather(IUserMessage umsg, string city, string country)
        {
            ...
            using (var http = new HttpClient())
                response = await http.GetStringAsync($"http://api.ninetales.us/Faultybot/weather/?city={city}&country={country}").ConfigureAwait(false);
        ...
        public async Task I(IUserMessage umsg, [Remainder] string query = null)
        {
            ...
            try
            {
                using (var http = new HttpClient())
                {
                    var reqString = $"https://www.googleapis.com/customsearch/v1?q={Uri.EscapeDataString(query)}&cx=018084019232060951019%3Ahs5piey28-e&num=1&searchType=image&fields=items%2Flink&key={FaultyBot.Credentials.GoogleApiKey}";
        ...
        public async Task Shorten(IUserMessage msg, [Remainder] string arg)
        {
            ...
            await msg.Channel.SendMessageAsync(await FaultyBot.Google.ShortenUrl(arg).ConfigureAwait(false));
        }
...
{items[0]["editions"][0]["image_url"].ToString()}";
        ...
        public async Task Hearthstone(IUserMessage umsg, [Remainder] string name = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(FaultyBot.Credentials.MashapeKey))
            {
            ...
            {
                http.DefaultRequestHeaders.Clear();
                http.DefaultRequestHeaders.Add("X-Mashape-Key", FaultyBot.Credentials.MashapeKey);
...
`URL:` {await FaultyBot.Google.ShortenUrl(found["url"].ToString()).ConfigureAwait(false)}";
                    await channel.SendMessageAsync(response);
                }
                catch
                {
