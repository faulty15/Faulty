            ...
            try
            {
                using (var http = new HttpClient())
                {
                    var data = JObject.Parse(await http.GetStringAsync($"http://api.champion.gg/stats/champs/mostBanned?" +
                                                    $"api_key={FaultyBot.Credentials.LoLApiKey}&page=1&" +
                                                    $"limit={showCount}")
                    ...
                    sb.AppendLine($"`{trashTalk[new FaultyRandom().Next(0, trashTalk.Length)]}`");
...
//                                  return;
//                              }
//                          var allData = JArray.Parse(await Classes.http.GetStringAsync($"http://api.champion.gg/champion/{name}?api_key={FaultyBot.Credentials.LOLAPIKey}").ConfigureAwait(false));
//                          JToken data = null;
//                          if (role != null)
//                          {
...
//                                                                                               $"champs/{name}?api_key={FaultyBot.Credentials.LOLAPIKey}")
