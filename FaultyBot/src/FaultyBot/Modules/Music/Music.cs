...
namespace FaultyBot.Modules.Music
{
    [FaultyModule("Music", "!!", AutoLoad = false)]
    public partial class Music : DiscordModule
    {
        ...
        public async Task SoundCloudPl(IUserMessage umsg, [Remainder] string pl)
        {
            ...
            {
                var scvids = JObject.Parse(await http.GetStringAsync($"http://api.soundcloud.com/resolve?url={pl}&client_id={FaultyBot.Credentials.SoundCloudClientId}").ConfigureAwait(false))["tracks"].ToObject<SoundCloudVideo[]>();
        ...
        public async Task DeletePlaylist(IUserMessage umsg, [Remainder] int id)
        {
            ...
            try
            {
                using (var uow = DbHandler.UnitOfWork())
                {
                    ...
                    if (pl != null)
                    {
                        if (FaultyBot.Credentials.IsOwner(umsg.Author) || pl.AuthorId == umsg.Author.Id)
                        {
                ...
                mp.OnCompleted += async (s, song) =>
                {
                    if (song.PrintStatusMessage)
                    {
                        try
                        {
                            ...
                            {
                                await QueueSong(queuer.Guild.GetCurrentUser(), textCh, voiceCh, (await FaultyBot.Google.GetRelatedVideosAsync(song.SongInfo.Query, 4)).ToList().Shuffle().FirstOrDefault(), silent, musicType).ConfigureAwait(false);
                            }
                        }
                        catch { }
                    }
