...
namespace FaultyBot.Modules.Music.Classes
{
    public class SoundCloud
    {
        ...
        public async Task<SoundCloudVideo> ResolveVideoAsync(string url)
        {
            ...
            if (string.IsNullOrWhiteSpace(FaultyBot.Credentials.SoundCloudClientId))
                throw new ArgumentNullException(nameof(FaultyBot.Credentials.SoundCloudClientId));

            string response = "";

            using (var http = new HttpClient())
            {
                response = await http.GetStringAsync($"http://api.soundcloud.com/resolve?url={url}&client_id={FaultyBot.Credentials.SoundCloudClientId}").ConfigureAwait(false);
        ...
        public async Task<SoundCloudVideo> GetVideoByQueryAsync(string query)
        {
            ...
            using (var http = new HttpClient())
            {
                response = await http.GetStringAsync($"http://api.soundcloud.com/tracks?q={Uri.EscapeDataString(query)}&client_id={FaultyBot.Credentials.SoundCloudClientId}").ConfigureAwait(false);
            }
    ...
    public class SoundCloudVideo
    {
        ...
        [JsonIgnore]
        public string StreamLink => $"https://api.soundcloud.com/tracks/{Id}/stream?client_id={FaultyBot.Credentials.SoundCloudClientId}";
    }
