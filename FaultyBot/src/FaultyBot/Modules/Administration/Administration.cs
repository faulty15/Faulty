...
using System.IO;
using static FaultyBot.Modules.Permissions.Permissions;
using System.Collections.Concurrent;

namespace FaultyBot.Modules.Administration
{
    [FaultyModule("Administration", ".")]
    public partial class Administration : DiscordModule
    {
        ...
        {
            FaultyBot.CommandHandler.CommandExecuted += DelMsgOnCmd_Handler;
        }
        ...
        static Administration()
        {
            ...
            {
                var configs = FaultyBot.AllGuildConfigs;
        ...
        {
            const string defaultMuteRoleName = "Faulty-mute";
        ...
        public async Task Setname(IUserMessage umsg, [Remainder] string newName)
        {
            ...
            await (await FaultyBot.Client.GetCurrentUserAsync()).ModifyAsync(u => u.Username = newName).ConfigureAwait(false);
        ...
        public async Task SetAvatar(IUserMessage umsg, [Remainder] string img = null)
        {
            ...
            using (var http = new HttpClient())
            {
                using (var sr = await http.GetStreamAsync(img))
                {
                    ...
                    await (await FaultyBot.Client.GetCurrentUserAsync().ConfigureAwait(false)).ModifyAsync(u => u.Avatar = imgStream).ConfigureAwait(false);
                }
            }
        ...
        public async Task SetGame(IUserMessage umsg, [Remainder] string game = null)
        {
            ...
            await FaultyBot.Client.SetGame(game).ConfigureAwait(false);
        ...
        public async Task SetStream(IUserMessage umsg, string url, [Remainder] string name = null)
        {
            ...
            await FaultyBot.Client.SetStream(name, url).ConfigureAwait(false);
        ...
        public async Task Send(IUserMessage umsg, string where, [Remainder] string msg = null)
        {
            ...
            var server = FaultyBot.Client.GetGuilds().Where(s => s.Id == sid).FirstOrDefault();
        ...
        public async Task Donators(IUserMessage umsg)
        {
            ...
            FaultySupportServer = FaultySupportServer ?? FaultyBot.Client.GetGuild(117523346618318850);

            if (FaultySupportServer == null)
                return;

            var patreonRole = FaultySupportServer.GetRole(236667642088259585);
            if (patreonRole == null)
                return;

            var usrs = FaultySupportServer.GetUsers().Where(u => u.Roles.Contains(patreonRole));
