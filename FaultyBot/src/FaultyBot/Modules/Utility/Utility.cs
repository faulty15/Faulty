...
namespace FaultyBot.Modules.Utility
{
    ...
    public partial class Utility : DiscordModule
    {
        ...
        public async Task Stats(IUserMessage umsg)
        {
            ...
            await channel.EmbedAsync(
                new Embed()
                {
                    ...
                    {
                        Name = $"FaultyBot v{StatsService.BotVersion}",
                        Url = "http://Faultybot.readthedocs.io/en/latest/",
                    ...
                    Fields = new[] {
                        ...
                        new EmbedField() {
                            Name = "Bot ID",
                            Value = FaultyBot.Client.GetCurrentUser().Id.ToString(),
                            Inline = true
                        },
                        new EmbedField() {
                            Name = "Commands Ran",
                        ...
                        new EmbedField() {
                            Name = "Presence",
                            Value = $"{FaultyBot.Client.GetGuilds().Count} servers\n{stats.TextChannels} Text Channels\n{stats.VoiceChannels} Voice Channels",
                            Inline = true
                        },
        ...
        public async Task ListServers(IUserMessage imsg, int page = 1)
        {
            ...
            var guilds = FaultyBot.Client.GetGuilds().OrderBy(g => g.Name).Skip((page - 1) * 15).Take(15);
