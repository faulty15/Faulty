...
namespace FaultyBot.Modules.CustomReactions
{
    [FaultyModule("CustomReactions",".")]
    public class CustomReactions : DiscordModule
    {
        ...
        public async Task AddCustReact(IUserMessage imsg, string key, [Remainder] string message)
        {
            ...
            if ((channel == null && !FaultyBot.Credentials.IsOwner(imsg.Author)) || (channel != null && !((IGuildUser)imsg.Author).GuildPermissions.Administrator))
            {
