...
namespace FaultyBot.Modules.ClashOfClans
{
    [FaultyModule("ClashOfClans", ",")]
            ...
            using (var uow = DbHandler.UnitOfWork())
            {
                ClashWars = new ConcurrentDictionary<ulong, List<ClashWar>>(
                    uow.ClashOfClans
                        ...
                        .Select(cw =>
                        {
                            cw.Channel = FaultyBot.Client.GetGuild(cw.GuildId)
        ...
        public static async Task<ClashWar> CreateWar(string enemyClan, int size, ulong serverId, ulong channelId)
        {
            using (var uow = DbHandler.UnitOfWork())
            {
                var cw = new ClashWar
                {
                    ...
                    GuildId = serverId,
                    ChannelId = channelId,
                    Channel = FaultyBot.Client.GetGuild(serverId)
